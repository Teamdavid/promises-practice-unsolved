...
export function handlePromise2(promiseArr) {
  return Promise.any(promiseArr).then((data) => data).catch((err) => err);
}
...
// Your code goes here...
export function handlePromise3(promiseArr) {
  return Promise.allSettled(promiseArr).then((results) => results);
}
...
export const newPromiseArr = promiseArr.filter((promise) => promise);
